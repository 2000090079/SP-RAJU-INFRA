...
function Admin() {
    ...
    const handleSubmit = async (e) => {
        ...
        // Stringify the array for Multer/Express
        ...
        try {
            ...
            if (editId) {
                url = `http://localhost:5000/projects/${editId}`
                method = "PUT"
            }

            const res = await fetch(url, {
                method,
                body: formData // Note: No 'Content-Type' header here, browser sets it for FormData
            })
    ...
    const editProject = (project) => {
        ...
        if (project.images && project.images.length > 0) {
            setPreview(project.images.map(img => `http://localhost:5000${img}`))
        } else {
            setPreview([])
        }
    }
    ...
    return (
        <div className="admin-wrapper">
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "20px" }}>
                ...
                <button onClick={logout} className="logout-btn" style={{ padding: "8px 16px", background: "#e74c3c", color: "white", border: "none", borderRadius: "5px", cursor: "pointer" }}>
                    Logout
                </button>
            </div>
                    ...
                    <form onSubmit={handleSubmit}>
                        ...
                        <div className="bhk-boxes">
                            {["2BHK", "3BHK", "4BHK", "5BHK"].map(bhk => (
                                <div key={bhk} className={`bhk-box ${bhkTypes.includes(bhk) ? "active" : ""}`} onClick={() => toggleBHK(bhk)}>
                                    {bhk}
                                </div>
                            ))}
                        </div>
        ...
        >
            {type}
                        ...
                        <div style={{ display: "flex", gap: "10px", marginTop: "20px" }}>
                            <button type="submit" style={{ flex: 2 }}>{editId ? "Update Project" : "Add Project"}</button>
                            {editId && <button type="button" onClick={resetForm} style={{ flex: 1, background: "#95a5a6" }}>Cancel</button>}
                        </div>
                    </form>
                </div>
                        ...
                        <div key={project._id} className="admin-project-card">
                            <div className="project-info">
                                {project.images && project.images[0] && (
                                    <img src={`http://localhost:5000${project.images[0]}`} className="admin-project-image" alt="thumb" />
                                )}
                                <div>
                                    <h4>{project.title}</h4>
